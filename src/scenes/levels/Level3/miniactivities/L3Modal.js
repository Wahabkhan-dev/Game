...
export function applyL3Frame(scene, depth = 1) {
  ...
  const img = key ? scene.add.image(W / 2, H / 2, key).setDisplaySize(W, H).setDepth(depth) : null;
  // Standalone menu button — these hospital mini-games have no persistent
  // header, so this is the only way to pause/exit mid-activity.
