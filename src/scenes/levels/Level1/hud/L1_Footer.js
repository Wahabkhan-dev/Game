...
import { THEME, makePanel, makeButton } from './L1_HudTheme.js';

// ════════════════════════════════════════════════════════════════════════════
// L1_Footer — the premium fantasy BOTTOM HUD for Level 1.
//
// The footer now contains ONLY one centered carved-wood Checkpoint Progress Bar
// (gold border, glow, shadow, thin fill, evenly-spaced Z1/Z2/Z3 + finish circles
// with diamond nodes and 🐾). The four circular control buttons were removed.
//
// A single contextual Attack (⚔️) button still exists but stays hidden until the
// boss fight (it is not part of the persistent footer, and lets the boss be
// finished with a click as well as the F key). Style is unchanged — only layout.
// ════════════════════════════════════════════════════════════════════════════
...
// centered tracker with equal left/right margins (≈25% shorter than full width)
const BAR_L = 120, BAR_R = W - 120, BAR_W = BAR_R - BAR_L;
const BAR_Y = H - 26;

// contextual boss attack button (hidden until the boss fight)
const BTN   = 38;
const ICON  = 0.5;
const ATK_X = W - M_SIDE - BTN / 2;
const ATK_Y = H - M_BOT - BTN - 26;

export class L1Footer {
  constructor(scene) {
    this.scene = scene;
  }

  build() {
    // tracker in `c`; the contextual attack button in `bc` (higher depth)
  ...
  _buildAttackButton() {
    ...
    const glow = s.add.image(cx, cy, 'l1hud_btn').setDisplaySize(BTN + 12, BTN + 12)
      .setScrollFactor(0).setDepth(D - 1).setTint(0xffd24a).setAlpha(0);
    const img = makeButton(s, cx, cy, BTN, D);
    const icon = s.add.image(cx, cy, 'l1hud_ic_attack').setScale(ICON).setScrollFactor(0).setDepth(D + 1);
  ...
  // ── The one and only footer element: a centered checkpoint progress bar ──────
  buildProgressBar(worldW) {
    const s = this.scene;
    const TY = BAR_Y;

    // shadow + slim wood track (clean rounded end caps, nothing extends past them)
    const sh = s.add.graphics().setScrollFactor(0).setDepth(D - 1);
    ...
    // dark groove (inset from the caps so the rounded gold border stays clean)
    ...
    // fill line (driven by Level1Scene.update via scene._zpFill)
    const zpFill = s.add.rectangle(BAR_L, TY, 2, 3, THEME.greenHP, 1)
      .setScrollFactor(0).setDepth(D + 2).setOrigin(0, 0.5);
    this.c.add(zpFill);

    // checkpoints — EVENLY spaced across the tracker
    const zones = [
    ...
    const xAt = (i) => BAR_L + (i / (zones.length - 1)) * BAR_W;
    ...
    zones.forEach((z, i) => {
      const bx = xAt(i);
      const glow = s.add.circle(bx, TY, 8, z.color, 0.28).setScrollFactor(0).setDepth(D + 2);
    ...
    // 🐾 runner marker (driven by scene._zpRunner)
    const zpRunner = s.add.text(BAR_L, TY - 2, '🐾', { fontSize: '11px' })
      .setScrollFactor(0).setDepth(D + 4).setOrigin(0.5, 1);
    this.c.add(zpRunner);

    // publish the fields Level1Scene.update() drives (unchanged logic)
    s._zpFill   = zpFill;
    s._zpRunner = zpRunner;
    s._zpLeft   = BAR_L;
    s._zpWidth  = BAR_W;
    s._zpWorldW = worldW;
  }
