// ════════════════════════════════════════════════════════════════════════════
// L4_GlendaSkin — swaps Level 4's player VISUAL to Glenda (run/idle/jump art).
//
// Gameplay is untouched: the physics body keeps its exact original world size,
// so collisions/jumps/platforming feel identical.
//
// IMPORTANT: Level4Scene is a plain Phaser.Scene (not BaseLevelScene) and its
// own update() hardcodes anim keys 'gleeda_walk' / 'gleeda_idle_anim' /
// 'gleeda_jump_anim' directly on `this.player` (not `this.shadow`, and not
// via configurable _walkAnim/_idleAnim/_jumpAnim fields like Level 2). So this
// skin targets scene.player and OVERWRITES those exact existing anim keys with
// Glenda's real frames — the movement code then picks up the new art with zero
...
// is normalized to the SAME on-screen height — same technique as L2_GlendaSkin.js.
// ════════════════════════════════════════════════════════════════════════════
...
const RUN_KEY  = (i) => `l4glenda_run_${i}`;
const IDLE_KEY = 'l4glenda_idle';
const JUMP_KEY = 'l4glenda_jump';

export function preloadGlendaSkin(scene) {
  scene.load.on('loaderror', (f) => {
    if (f && f.key && String(f.key).startsWith('l4glenda_')) {
      console.error(`[L4 GlendaSkin] ❌ frame failed: ${f.key} → ${f.url}`);
    }
  });
...
export function applyGlendaSkin(scene) {
  ...
  if (!player) { console.warn('[L4 GlendaSkin] no player sprite (scene.player) — skipped'); return; }
  ...
  // Overwrite the EXISTING anim keys Level4's update() already calls directly
  // ('gleeda_walk' / 'gleeda_idle_anim' / 'gleeda_jump_anim') with Glenda's
  // real frames — no changes needed to the movement/update code.
  ...
  console.log(`[L4 GlendaSkin] applied — out ${outW}×${outH}, scale ${scale.toFixed(3)}, world body ${worldBW.toFixed(0)}×${worldBH.toFixed(0)} (gameplay preserved).`);
}
