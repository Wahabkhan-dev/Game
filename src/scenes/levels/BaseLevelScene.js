...
export class BaseLevelScene extends Phaser.Scene {
  ...
  _buildHUD(config) {
    ...
    const _ban = buildLevelBanner(this, '', config.chapterName || '', 28);
    this._hudChapter = _ban.titleTxt;
    ...
    // Premium wood/gold hearts panel — same look as Level 1 (shared HUD theme)
    generatePremiumHudTextures(this);
    makePanel(this, 4, 4, 94, 50, 28);

    this._hearts = [];
    for (let i = 0; i < 3; i++) {
      const h = this.add.image(19 + i * 27, 19, 'shud_heart').setScale(0.55).setScrollFactor(0).setDepth(30);
  ...
  _openPauseMenu() {
    ...
    // Finalized premium wood/gold modal panel (same art system as the header)
    generatePremiumHudTextures(this);
    const shadowG = this.add.graphics().setScrollFactor(0).setDepth(95);
    shadowG.fillStyle(0x000000, 0.4); shadowG.fillRoundedRect(px + 3, py + 5, PW, PH, 16);
    toDestroy.push(shadowG);
    toDestroy.push(makePanel(this, px, py, PW, PH, 96));
    toDestroy.push(this.add.image(px + 22, py + 30, 'shud_leaf').setScale(0.5).setScrollFactor(0).setDepth(97));
    toDestroy.push(this.add.image(px + PW - 22, py + 30, 'shud_leaf').setScale(0.5).setScrollFactor(0).setDepth(97).setFlipX(true));
    ...
    divG.lineStyle(1, THEME.GOLD, 0.4);
    divG.lineBetween(px + 22, py + 44, px + PW - 22, py + 44);
    toDestroy.push(divG);
  ...
  updateMovement() {
    if (this._isDying || this._puzzleActive || this._pauseMenuOpen || this._miniGameOpen || this._freezeForMini) return;
  ...
  // ── All lives lost — default behavior shared by every level. Override this
  ...
  // ── Full-screen video overlay (game-over cinematics, feeding scenes, etc.) ──
  // Plays `key` (preloaded via this.load.video in BootScene) centered and
  // letterboxed over the current scene, then calls onDone. Physics is paused
  // while it plays. A skip button + safety-net timer mean a missing or
  // never-firing video can never soft-lock the game.
  _playVideoOverlay(key, onDone, opts = {}) {
    if (this._videoOverlayOpen) return;
    ...
    if (!this.cache.video.exists(key)) { finish(); return; }

    video = this.add.video(W / 2, H / 2, key).setScrollFactor(0).setDepth(201);
    video.on('created', () => {
    ...
    const safety = Number.isFinite(opts.maxMs) && opts.maxMs > 0
      ? this.time.delayedCall(opts.maxMs, finish)
      : null;
  ...
  _launchMiniGame(folder, onComplete) {
    ...
    this.physics.pause();
    // Stick the character exactly where the activity triggered — zero its
    // velocity so it can't drift forward (also avoids a lurch on resume).
    if (this.shadow?.body) this.shadow.setVelocity(0, 0);
