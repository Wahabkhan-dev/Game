...
export function generateL6Assets(scene) {
  // Guard — skip if already generated this session (key never loaded as a real image)
  if (scene.textures.exists('l6_doghouse')) return;
  ...
  if (!scene.textures.exists('l6_sparkle')) {
    g.clear();
    ...
    gen('l6_sparkle', 32, 32);
  }
  if (!scene.textures.exists('l6_hint_bulb')) {
    g.clear();
    ...
    gen('l6_hint_bulb', 40, 40);
  }
  if (!scene.textures.exists('l6_coin')) {
    g.clear();
  ...
  if (!scene.textures.exists('l6_diamond')) {
    g.clear();
...
export function generateL5StreetAssets(scene) {
  ...
  // create the few procedural-only textures (l6_homesign, l6_star).
  if (scene.textures.exists('l6_homesign')) return;
  const g = scene.make.graphics({ add: false });
