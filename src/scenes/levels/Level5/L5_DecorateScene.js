...
// ── Level 5 · Build Scene — identical to L4_Decorate but ends at Level5 treatment
const HX = 470, HY = 374;
...
const STEPS = [
  ...
  { need: 'bed',   label: 'Add the Cozy Bed',     reveal: 1.00, prop: 'l5_bed' },
  { need: 'bowl',  label: 'Set the Food Bowl',    reveal: 1.00, prop: 'l5_food_bowl' },
];

const TRAY = [
  { key: 'wood',  tex: 'l5_wood',      label: 'Wood' },
  { key: 'roof',  tex: 'l5_roof',      label: 'Roof' },
  { key: 'nails', tex: 'l5_nails',     label: 'Nails' },
  { key: 'paint', tex: 'l5_paint',     label: 'Paint' },
  { key: 'bed',   tex: 'l5_bed',       label: 'Bedding' },
  { key: 'bowl',  tex: 'l5_food_bowl', label: 'Bowl' },
];

export class L5_DecorateScene extends Phaser.Scene {
  constructor() { super('L5_Decorate'); }
  ...
  create(data) {
    ...
    // Gold border frame (shared Level-1 modal art) — decorative, never blocks input
    applyModalFrame(this, 1);
    ...
    this._gamma = this.add.image(140, HY + 4, this.textures.exists('gemma_idle') ? 'gemma_idle' : 'l5_food_bowl')
      .setOrigin(0.5, 1).setDisplaySize(140, 80).setDepth(8);
    this._gammaScale = this._gamma.scaleX;
    this._gammaBob = this.tweens.add({ targets: this._gamma, y: HY, duration: 700, yoyo: true, repeat: -1, ease: 'Sine.easeInOut' });

    const src = this.textures.get('l5_house_finished').getSourceImage();
    ...
    this._houseImg = this.add.image(HX, HY, 'l5_house_finished').setOrigin(0.5, 1).setDisplaySize(this._hw, this._hh).setDepth(5);

    this._stage = 0; this._reveal = 0; this._busy = false;
    this._setReveal(0);
    this._buildTray();
    this._step = 0;
    this._startStep();
  ...
  _startStep() {
    ...
    this._needed = st.need; this._busy = false;
  ...
  _pick(slot) {
    ...
    slot.used = true; slot.hit.disableInteractive();
  ...
  _startNails(st) {
    ...
    spots.forEach(([nx, ny]) => {
      ...
      ring.once('pointerdown', () => {
        ...
        this.cameras.main.shake(80, 0.004); this._sparkle(nx, ny);
  ...
  _startPaint(st) {
    ...
    const paintMove = (p) => {
      ...
      if (prog >= 1) { this.input.off('pointermove', paintMove); brush.destroy(); zone.destroy(); this._applyStep(st); }
    };
  ...
  _applyStep(st) {
    ...
    this._stage++;
    this.cameras.main.flash(180, 80, 200, 90); this._sparkle(HX, this._houseTop + 20);
    ...
    if (st.prop && this.textures.exists(st.prop)) {
      ...
      let ph, px, py, depth;
      if (st.prop === 'l5_bed') { ph = 30; px = HX - this._hw * 0.09; py = HY - this._hh * 0.20; depth = 6; }
      else { ph = 42; px = HX - this._hw * 0.04; py = HY + 10; depth = 9; }
  ...
  _finish() {
    ...
    if (this._gammaBob) { this._gammaBob.stop(); this._gammaBob = null; }
  ...
  _proceed() {
    ...
    this.add.text(W / 2, H / 2 - 92, 'House Ready! 🏠', { fontSize: '20px', fontFamily: 'Georgia, serif', color: '#d94060', stroke: '#fdf0e8', strokeThickness: 2 }).setOrigin(0.5).setDepth(62);
    ['⭐', '⭐', '⭐'].forEach((s, i) => {
  ...
  _sparkle(x, y) {
    for (let i = 0; i < 8; i++) {
      ...
      const s = this.add.image(x, y, this.textures.exists('l5_sparkle') ? 'l5_sparkle' : 'l5_coin').setScale(0.6).setDepth(63);
